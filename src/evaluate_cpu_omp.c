...
conjugrad_float_t evaluate_cpu_omp(
    ...
    unsigned long ncol = ud->ncol;
    unsigned long nrow = ud->nrow;
	...
	for(unsigned long i = 0; i < nrow; i++) {
		...
		for(unsigned long a = 0; a < N_ALPHA-1; a++) {
			for(unsigned long s = 0; s < ncol; s++) {
				OMP_PC(i,a,s) = V(s,a);
			}
		}
        ...
        for(unsigned long k = 0; k < ncol; k++) {
			...
			for(unsigned long a = 0; a < N_ALPHA * ncol; a++) {
				*pp++ += *pw++;
			}
		}

		// compute precomp_sum(s) = log( sum(a=1..21) exp(PC(a,s)) )
		for(unsigned long a = 0; a < N_ALPHA; a++) {
			for(unsigned long s = 0; s < ncol; s++) {
	...
	for(unsigned long b = 0; b < N_ALPHA; b++) {
		for(unsigned long k = 0; k < ncol; k++) {
			for(unsigned long a = 0; a < N_ALPHA; a++) {
				for(unsigned long j = 0; j < ncol; j++) {
	...
	for(unsigned long v = 0; v < nsingle; v++) {
		reg += lambda_single * x[v] * x[v];
		g[v] += F2 * lambda_single * x[v]; // F2 is 2.0
	}

	for(unsigned long v = nsingle_padded; v < nvar_padded; v++) {
